...
export async function getUserData (userName) {
	...
	if (userData.stream !== null) {
		result = {
			userName: userData.stream.channel.name,
			gameName: userData.stream.game,
			viewers: userData.stream.viewers,
			channelDescription: userData.stream.channel.description,
			channelLogoURL: userData.stream.channel.logo,
			streamPreviewURL: userData.stream.preview.large
		}
	}
	return result
}
