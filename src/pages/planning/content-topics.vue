	...
	methods: {
		...
		saveAddTopicButtonPressed: async function () {
			if (this.contentTopicList.filter(item => item.id === this.contentTopicID).length > 0) {
				...
				this.contentTopicList.push({
					id: this.contentTopicID,
					name: this.contentTopicName,
					imgUrl: this.contentTopicImageURL,
					platforms: this.contentTopicPlatformsSelected,
					description: this.contentTopicDescription,
					extLink1: this.contentTopicExternalLink,
					extLink2: this.contentTopicExternalLink2
				})
				this.contentTopicID = ''
